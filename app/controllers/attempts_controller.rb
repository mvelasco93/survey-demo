class AttemptsController < ApplicationController
  ...
  def create
    ...
    if @attempt.valid? && @attempt.save
      correct_options_text = @survey.correct_options.present? ? 'Abajo están las respuestas correctas. Están marcadas con verde' : ''
      redirect_to attempt_path(@attempt.id), notice: "Gracias por contestar #{@survey.name}! #{correct_options_text}"
    else
      build_flash(@attempt)   
      @participant = current_user
      render :new
